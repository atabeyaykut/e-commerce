...
import { teamPageData } from '../data/teamPageData';

const TeamPage = () => {
  const { header, gridImages } = teamPageData;
  const mainImage = gridImages.find(img => img.gridSpan === 'large');
  const smallImages = gridImages.filter(img => img.gridSpan === 'small');
  ...
  const teamMembers = [
    {
      ...
      social: {
        ...
        email: 'mailto:gn.ozdemir@gmail.com'
      }
    },
    {
      id: 2,
      ...
      social: {
        ...
        email: 'mailto:atabeyaykutt@gmail.com'
      }
    },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 w-full">
        ...
        <div className="lg:col-span-2 grid grid-cols-2">
          {smallImages.map((image) => (
            <div key={image.id} className="relative h-[200px] lg:h-85">
              <img
                src={image.src}
                alt={image.alt}
                className="absolute inset-0 w-full h-full object-cover"
